...
const Media = ({ groupId, UserAuth }) => {
    ...
    const { albumsGroup, getGroupAlbums, getGroupMedia } = useMediaContext();
    const albums = albumsGroup;
    useEffect(() => {
        if (AlbumId) {
            (async function () {
                setLoading(true);
                const media = await getGroupMedia(groupId, AlbumId.albums.id);
                setAlbumData(media);
                setLoading(false);
            })()
        }
    ...
    const fetchData = async () => {
        await getGroupAlbums(groupId);
        const media = await getGroupMedia(groupId);
        setMedia(media);
        FilterData(media);
    };
    ...
    return (
        <div className="personal_right_box">
                ...
                <div className='row'>
                    <div className="col-lg-4 col-md-6 col-12">
                        <div className="search-groups-box">
                            <form>
                                <input type="text" placeholder="Search Media..." onChange={(e) => { setSearch(e.target.value) }} />
                    ...
                    <div className="col-lg-4 col-md-6 col-12">
                        {UserAuth && <div className='btn-wrpttr'>
            ...
            {uploadMedia && <UploadMedia albums={albums} user_id={groupId} fetchData={fetchData} getAlbums={getGroupAlbums} setUploadMedia={setUploadMedia} groupId={groupId} />}
            <div className="tab-content" id="pills-tabContent">
                    ...
                    <div className='emptyBox'>
                        ...
                        {media == null ? <div className='col-md-7 mt-5 pt-5'><div className='loader_Post' /></div> : (media.length > 0 ?
                            <div className='row col-md-8 mt-4'>{
                                media.map((med) => (
                                    <div className='col-lg-4 col-md-6'><MediaBox media={med} groupId={groupId} fetchData={fetchData} /></div>
                                ))
                            }</div>
                            :
                ...
                <div className="tab-pane fade" id="pills-profile" role="tabpanel" aria-labelledby="pills-profile-tab">
                    <div className='emptyBox'>
                        ...
                        {(AlbumId && auth?.id == AlbumId.albums.user_id) && <AlbumOptionsDropdown setAlbumData={setAlbumData} setEdit={setEdit} setAlbumId={setAlbumId} fetchData={fetchData} AlbumId={AlbumId} />}
                        {!edit && <h3>{AlbumId ? AlbumId.albums.title : 'Album List'}</h3>}
                        {edit ? <EditAlbum setEdit={setEdit} getAlbums={getGroupAlbums} groupId={groupId} EditData={AlbumId} /> : <>
                            {
            ...
            {showAddPopup && <AddAlbum setAddPopup={setAddPopup} getAlbums={getGroupAlbums} userId={auth?.id} groupId={groupId} />}
        </div>
    )
}
